  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/data', async (req, res) => {
        ...
        try {
          const ip = getClientIp(req)
          const { action = 'select', table, select, filter, order, limit, payload, match } = await readJsonBody(req)
          ...
          if (!isLisTable && !isReadOnlyHisTable) return sendJson(res, 400, { success: false, error: 'Invalid table name' })
          ...
          if (action !== 'select') {
            ...
            if (!hasPermission(session.r, table, action)) {
              return sendJson(res, 403, { success: false, error: `Forbidden: role "${session.r}" cannot ${action} ${table}` })
            }
            const bucket = checkRateLimit(`mut:${ip}:${session.uid || 'anon'}:${action}:${table}`, { limit: 60, windowMs: 60 * 1000 })
          ...
          if (action === 'insert') {
            ...
            method = 'PATCH'
            path += `?${match || filter}&select=${select || '*'}`
            body = JSON.stringify(payload)
