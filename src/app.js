...
const ORDER_FILE_DB = 'lis-one-order-result-files';
const ORDER_FILE_STORE = 'files';

function openOrderFileDb() {
    return new Promise((resolve, reject) => {
        const req = indexedDB.open(ORDER_FILE_DB, 1);
        req.onupgradeneeded = () => {
            const db = req.result;
            if (!db.objectStoreNames.contains(ORDER_FILE_STORE)) {
                const store = db.createObjectStore(ORDER_FILE_STORE, { keyPath: 'id' });
...
async function getOrderFiles(orderId) {
    const db = await openOrderFileDb();
    return new Promise((resolve, reject) => {
        const tx = db.transaction(ORDER_FILE_STORE, 'readonly');
        const store = tx.objectStore(ORDER_FILE_STORE);
...
async function saveOrderFiles(orderId, files) {
    const db = await openOrderFileDb();
    ...
    await new Promise((resolve, reject) => {
        const tx = db.transaction(ORDER_FILE_STORE, 'readwrite');
        const store = tx.objectStore(ORDER_FILE_STORE);
...
async function getOrderFileById(fileId) {
    const db = await openOrderFileDb();
    return new Promise((resolve, reject) => {
        const tx = db.transaction(ORDER_FILE_STORE, 'readonly');
        const req = tx.objectStore(ORDER_FILE_STORE).get(fileId);
...
window.calculateCart = () => renderOrderSummary();
...
async function hydrateOrderUploadBadges(orders) {
    await Promise.all((orders || []).map(async (order) => {
        ...
        if (!badge) return;
        const files = await getOrderFiles(order.order_id);
    ...
    return files.map(file => `
        <div class="order-upload-item">
            ...
            <div class="d-flex gap-1">
                <button class="btn btn-sm btn-outline-primary" onclick="openUploadedResultFile('${escapeHtml(file.id)}')"><i class="bi bi-eye"></i> View</button>
                <button class="btn btn-sm btn-outline-success" onclick="downloadUploadedResultFile('${escapeHtml(file.id)}')"><i class="bi bi-download"></i> Download</button>
            </div>
        </div>
    `).join('');
}
    ...
    list.innerHTML = files.map(file => `
        <div class="upload-manager-item">
            <div class="upload-file-main">
                <i class="bi ${fileKindIcon(file)}"></i>
                <div>
                    <b>${escapeHtml(file.name)}</b>
                    <small>${escapeHtml(file.type || 'file')} · ${formatFileSize(file.size)} · ${formatDateTime(file.uploadedAt)}</small>
                </div>
            </div>
            <div class="upload-file-actions">
                <button class="btn btn-sm btn-outline-primary" onclick="openUploadedResultFile('${escapeHtml(file.id)}')"><i class="bi bi-eye-fill"></i> ເບິ່ງ</button>
                <button class="btn btn-sm btn-outline-success" onclick="downloadUploadedResultFile('${escapeHtml(file.id)}')"><i class="bi bi-download"></i> ໂຫຼດ</button>
            </div>
        </div>
    `).join('');
}
...
document.addEventListener('change', async (event) => {
    ...
    try {
        const saved = await saveOrderFiles(orderId, input.files);
        await hydrateOrderUploadBadges(recentOrders);
        await renderOrderUploadModal(orderId);
        Swal.fire({
            icon: 'success',
            title: 'ສຳເລັດ',
            text: `ອັບໂຫຼດໄຟລ໌ຜົນກວດ ${saved.length} ໄຟລ໌ແລ້ວ`,
            timer: 1500,
...
window.openUploadedResultFile = async function(fileId) {
    const file = await getOrderFileById(fileId);
    if (!file?.blob) return Swal.fire('ບໍ່ພົບໄຟລ໌', '', 'warning');
    window.open(URL.createObjectURL(file.blob), '_blank');
};

window.downloadUploadedResultFile = async function(fileId) {
    const file = await getOrderFileById(fileId);
    if (!file?.blob) return Swal.fire('ບໍ່ພົບໄຟລ໌', '', 'warning');
    const url = URL.createObjectURL(file.blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = file.name || 'result-file';
